    ...
    try:
        ...
        def create_member(self):
            ...
            while num in member_details_dict:
                ...
                bmi = float(input("Please enter body mass of Member!\n"))
                while(isinstance(bmi, float) == False):
                    bmi = float(input("Please enter a correct value for body mass of Member!\n"))
        ...
        def select_regimen(self, bmi):
            while(isinstance(bmi,float) == False):
                print("Sorry, Body mass entered is not a correct value!\n")
                break
            else:
                if(bmi < 18.5):
...
class Member:
    def __init__(self):
        ...
        if self.num in member_details_dict:
            print("Welcome, {} to Fitness gym!".format(member_details_dict[self.num]['Full Name']))
            self.member_menu()
        else:
